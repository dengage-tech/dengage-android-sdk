        ...
        try {
            if (first.data.isRealTime() != second.data.isRealTime()) {
                ...
                if (first.data.priority == second.data.priority) {
                    val firstHasRules = !first.data.displayCondition.displayRuleSet?.displayRules.isNullOrEmpty()
                    val secondHasRules = !second.data.displayCondition.displayRuleSet?.displayRules.isNullOrEmpty()
                    if (first.data.isRealTime() && second.data.isRealTime() &&
                        firstHasRules != secondHasRules
                    ) {
                        return if (firstHasRules && !secondHasRules) -1 else 1
                    } else {
