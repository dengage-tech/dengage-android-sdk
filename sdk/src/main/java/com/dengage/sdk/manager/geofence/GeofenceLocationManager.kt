...
import com.dengage.sdk.manager.geofence.GeofenceLocationContract as GLC
import com.dengage.sdk.util.ContextHolder as CH
import com.dengage.sdk.util.DengageLogger as DL
import com.dengage.sdk.manager.geofence.GeofencePermissionsHelper as GPH
import com.dengage.sdk.manager.geofence.GeofenceLocationReceiver as GLR
...
internal class GeofenceLocationManager : BaseMvpManager<GLC.View, GLC.Presenter>(), GLC.View {
    ...
    private fun startLocationUpdates() {
        if (!started || (LocationRequest.PRIORITY_BALANCED_POWER_ACCURACY != startedDesiredAccuracy) || (DMUI != startedInterval) || (FMUI != startedFastestInterval)) {
            ...
            lClient.requestLocationUpdates(
                locationRequest,
                GLR.getLocationPendingIntent(CH.context)
            )
            this.started = true
    ...
    private fun stopLocationUpdates() {
        lClient.removeLocationUpdates(GLR.getLocationPendingIntent(CH.context))
        started = false
    }
    ...
    private fun replaceSyncedGeofences(geofenceClusters: Array<Cluster>?) {
        ...
        gClient.addGeofences(request, GLR.getSyncedGeofencesPendingIntent(CH.context))
            .run {
                addOnSuccessListener {
    ...
    private fun removeBubbleGeofences() {
        gClient.removeGeofences(GLR.getBubbleGeofencePendingIntent(CH.context))
        DL.debug("Removed bubble geofences")
    }

    private fun removeSyncedGeofences() {
        gClient.removeGeofences(GLR.getSyncedGeofencesPendingIntent(CH.context))
